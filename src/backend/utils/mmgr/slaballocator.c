...
void SA_Deallocater(void* ptr)
{
    ...
    if (block == NULL) {
        fprintf(stderr, "SA_Deallocater: MemoryChunkGetBlock returned NULL for ptr=%p\n", ptr);
        return;
    }
    ...
    {
        fprintf(stderr,
                "SA_Deallocater: corrupted block header (instance=%p, cache=%p)\n",
                (void*)instance, (void*)cache);
        return;
    }
